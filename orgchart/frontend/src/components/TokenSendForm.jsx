...
export default function TokenSendForm({ token, onSend }) {
  const [description, setDescription] = useState(token?.description || '');
  const [comment, setComment] = useState('');
  const [loading, setLoading] = useState(false);

  // Здесь можно добавить селектор получателя, если нужно

  const handleSend = async () => {
    setLoading(true);
    // Здесь должна быть логика отправки токена (onSend или API)
    setTimeout(() => {
      setLoading(false);
      if (onSend) onSend({ description, comment });
    }, 1000);
  };

  return (
    <div className="flex flex-col h-full justify-center items-center p-4 min-h-[320px]">
